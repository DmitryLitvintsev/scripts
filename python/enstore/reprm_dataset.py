...
_URL = "https://samweb.fnal.gov:8483/sam/gm2/api/files/list/locations?format=plain&schema=root&defname=%s&filter_path=enstore"
...
def main():
    ...
    parser.add_argument(
        "--dataset",
        help="comma separated list of SAM datasets")

    parser.add_argument(
        "--file",
        help="file containing list of SAM datasets, one dataset per line")
    ...
    datasets = []
    if args.dataset:
        datasets = [i for i in args.dataset.strip().split(",")]

    if args.file:
        with open(args.file, "r") as f:
            datasets = [i.strip() for i in f]
    ...
    for dataset in datasets:
        ...
        r = session.get(_URL % (dataset,),  headers=headers, verify=False) 
        try: 
            r.raise_for_status()
            file_count = 0
            for i in r.iter_lines():
                ...
                file_count += 1
                queue.put(parsed.path)
            print_message("Processing dataset %s, number of files %d" % (dataset, file_count,))
        except:
            print_error("Failed to query dataset %s" % (dataset, ))
            continue
