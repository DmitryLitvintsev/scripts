...
def execute_admin_command(ssh, cmd):
    ...
    if len(stderr.readlines()) > 0:
        print_error("Command %s failed %s " % (cmd, " ".join(stderr.readlines())))
...
def set_sticky(ssh, pnfsid, duration=None):
    """
    marks pnfsid on all locations as precious
    """
    if not duration:
        ...
        result = execute_admin_command(ssh, "\sl " + pnfsid + " rep set sticky -o=admin -l=" + str(duration) + " " + pnfsid + " on")
    return True

def set_sticky_on_location(ssh, pool, pnfsid, duration=None):
    """
    marks pnfsid on all locations as precious
    """
    if not duration: 
        result = execute_admin_command(ssh, "\s " + pool + " rep set sticky -o=admin -l=1209600000 " + pnfsid + " on")
    else:
        result = execute_admin_command(ssh, "\s " + pool + " rep set sticky -o=admin -l=" + str(duration) + " " + pnfsid + " on")
        
    return True

def unpin(ssh, pnfsid, pool=None):
    """                                                                                                                                                                                                       
    marks pnfsid on all locations as precious                                                                                                                                                                 
    """
    if not pool:
        ...
        while True:
            ...
            while files:
                ...
                if not locations:
                    ...
                    right_locations = [ l for l in locations if l.startswith("rw-gm2") ]
                    set_sticky = False
                    for l in right_locations:
                        repls = None
                        try:
                            repls = rep_ls(ssh, l, pnfsid)
                        except RuntimeError as e:
                        ...
                        if repls:
                            try:
                                res = set_sticky_on_location(ssh, l, pnfsid, 7776000000)
                                set_sticky = True
                                break
                            except RuntimeError as e:
                                print_error("%s %s : Failed to set %s sticky on location %s" % (self.pool,
                                                                                                label,
                                                                                                pnfsid,
                                                                                                l))
                    ...
                    if set_sticky:
                        ...
                        files.append((bfid, pnfsid, crc, fsize))
                        wrong_locations =  [ l for l in locations if not l.startswith("rw-gm2") ]
                        ...
                        for l in wrong_locations:
                            ...
                            if repls:
                                ...
                                #    destination = "GM2Pools"
                                res = migrate(ssh, l, destination, pnfsid)
